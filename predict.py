...
for timeset in range(18,19):      #暂时设置为一个时间    
    ...
    for collocation_now in collocation_24hour_list[timeset-1]:
        ...
        # if collocation_now != 5:       #指定一两个搭配调试用
        #    continue
        ...
        # # print('beta的大小是',len(beta),len(beta[0]))
        # # print('训练集大小是',len(train_X),len(train_X[0]))
        # # print('泵组搭配是',collocation_now)
        ...
        length = int(betainfo[3][colloction_index])

        if row == 0:
            continue

        X = shttime.range('B'+str(row+1)+':F'+str(row+length)).value

        test_X_r = int(shttime.range('G'+str(row)).value)
        test_X = shttime.range('L'+str(row+1)+':P'+str(row+test_X_r)).value

        test_Y_raw = shttime.range('Q'+str(row+1)+':V'+str(row+test_X_r)).value

        train_X_r = int(shttime.range('E'+str(row)).value)
        train_X = shttime.range('G'+str(row+1)+':K'+str(row+train_X_r)).value

        ker = 'rbf'
        par = shttime.range('N'+str(row)).value

        beta = shttime.range('W'+str(row+1)+':AB'+str(row+train_X_r)).value

        R2 = shttime.range('R'+str(row)).value
        RMSE_output = shttime.range('S'+str(row)+':X'+str(row)).value
        ...
        print('凑形式：',test_Y_simulation,type(test_Y_simulation))
        ...
        if predict_simulation_output[0] > 50:
            predict_simulation_output[0] = 50
        if predict_simulation_output[1] > 50:
            predict_simulation_output[1] = 50
        ...
        bias = abs(( predict_simulation_output[4] / predict_simulation_output[2] * 1000 ) - predict_simulation_output[5])       #使用待预测数据的预测结果的单耗偏差，评价准确度
        # print(bias)

        # bias1 = sum( abs(np.array(Ypredtest)[:,5] - np.array(test_Y_raw)[:,5]) )
        # bias2 = sum( abs( np.array(Ypredtest)[:,4]/np.array(Ypredtest)[:,2]*1000 - np.array(test_Y_raw)[:,5]) )

        #泵组切换代价
        io = r'c:\Users\lz\Desktop\新提取表格.xls'      #获取文件绝对存储位置
        data_changecost_list = pd.read_excel(io, sheet_name =  '泵组更换代价')     #将工作簿读取到data，data为dataframe类型实例
        data_changecost_list = np.array(data_changecost_list)
        print(data_changecost_list)

        wb = app.books.open(r'c:\Users\lz\Desktop\新预测操作.xlsx')
        if timeset == 1:
            sht1 = wb.sheets["24"]
        if timeset > 1:
            sht1 = wb.sheets[str(timeset-1)]
        
        beforehour_collocation = sht1.range("D45").value     #获取上一时刻搭配
        collocation_changecost = getcost(data_changecost_list,beforehour_collocation,collocation_now)       #获取搭配切换代价
        print(collocation_changecost)

        #组成一条搭配所需要的输出数据
        predict_simulation_output = predict_simulation_output.tolist()
        each_collocation_output = predict_simulation_output[:]
        each_collocation_output_backup = [0,R2,bias,collocation_changecost]     #0为综合排序保留位置
        each_collocation_output.insert(0,collocation_now)
        each_collocation_output.extend(each_collocation_output_backup)       #合并列表
        each_collocation_output.extend(RMSE_output)
        print(each_collocation_output)

        all_collocation_output.append(each_collocation_output)
    ...
    wbbeta.save()
    wbbeta.close()
    print(all_collocation_output)

    #算出综合指标，排序
    sortorder_dh = np.array( sortorder([x[6] for x in all_collocation_output],0) )
    sortorder_R2 = np.array( sortorder([x[8] for x in all_collocation_output],1) )
    sortorder_bias = np.array( sortorder([x[9] for x in all_collocation_output],0) )
    sortorder_cost = np.array( sortorder([x[10] for x in all_collocation_output],0) )
    sortorder_comprehesive = (sortorder_dh*0.3 + sortorder_R2*0.15 + sortorder_bias*0.15 + sortorder_cost*0.4).tolist()

    #将综合指标加入输出列表
    all_collocation_output = np.array(all_collocation_output)
    all_collocation_output[:,7] = sortorder_comprehesive 
    all_collocation_output.tolist()

    all_collocation_output = sorted(all_collocation_output,key = lambda x:x[7])
    all_collocation_output_correct = []
    for i in all_collocation_output:
        i = i.tolist()
        all_collocation_output_correct.append(i)

    
    print(all_collocation_output_correct)

    #写入文件
    
    sht2 = wb.sheets[str(timeset)]
    sht2.range('D45:T56').options(expand='table').value = None
    sht2.range('D45').options(expand='table').value = all_collocation_output_correct     #从D45开始写入

    wb.save()
    wb.close()
    app.quit()
    print(timeset,'时刻预测推荐已经写入excel。')
    #print(np.array(collocation_24hour_list[0]))
print('所有时刻均已预测完成')
#collection()

#print('这是预测-真实：',bias1,'\n这是计算-真实：',bias2)
#print(Ypredtest[:,4],Ypredtest[:][2],Ypredtest[:][5])
#print(Ypredtest)
